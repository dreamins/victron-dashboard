...
# Stop leftover test containers only (not production)
"${COMPOSE_TEST[@]}" stop ble-decoder mosquitto influxdb 2>/dev/null || true
sleep 2

MQTT_BIND_IP=127.0.0.1 "${COMPOSE_TEST[@]}" up -d --build mosquitto influxdb ble-decoder \
...
for i in $(seq 1 20); do
    "${COMPOSE_TEST[@]}" logs ble-decoder 2>/dev/null | grep -q "MQTT connected" && break
    sleep 2
done

# ─── Replay fixture through decoder ───────────────────────────────────────────

run "${COMPOSE_TEST[@]}" exec -T ble-decoder python /app/replay.py \
...
if "${COMPOSE_TEST[@]}" ps ble-decoder 2>/dev/null | grep -qiE "up|running"; then
...
if "${COMPOSE_TEST[@]}" logs ble-decoder 2>/dev/null | grep -q "no key configured"; then
...
PROD=$(run "${COMPOSE_TEST[@]}" exec -T influxdb influx query --org home \
...
# Stop TEST influxdb, run replay (writes fail → decoder buffers), restart, verify flush
"${COMPOSE_TEST[@]}" stop influxdb 2>/dev/null
sleep 2
...
if "${COMPOSE_TEST[@]}" logs ble-decoder 2>/dev/null | grep -qE "retry in|influx write.*failed"; then
    pass "decoder logged retry attempts during outage"
else
    fail "decoder did not log retry attempts (buffer/retry may not be working)"
fi

# Bring TEST influxdb back
"${COMPOSE_TEST[@]}" start influxdb 2>/dev/null
