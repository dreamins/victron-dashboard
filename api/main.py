...
"""
        ...
        for d in result.values():
            if d["ts"]: d["ts"] = d["ts"].isoformat()
        return result

    def get_history(self, device: str, field: str, start_s: int, interval: str) -> Dict[str, Any]:
...
"""
  ...
  from(bucket: "{bucket}")
    ...
    |> keep(columns: ["_time", "_value"])
        ...
        # Deduplicate at bucket boundaries
        seen_t: Dict[str, Dict] = {}
        for p in raw_points: seen_t[p["t"]] = p
...
def history(
    ...
    if not _ID_RE.match(device): raise HTTPException(400, "invalid device")
    if field not in VALID_FIELDS: raise HTTPException(400, "invalid field")
    if not _DURATION_RE.match(interval): raise HTTPException(400, "invalid interval")
    try:
        range_s = _parse_s(start)
    except ValueError:
        raise HTTPException(400, "invalid start")
    return repo.get_history(device, field, range_s, interval)
